...
async def check_after_not_joined(bot, uid):
    """Set-2 (channels_after) join check karo."""
    not_joined = []
    for ch in get_channels("channels_after"):
        try:
            m = await bot.get_chat_member(ch["id"], uid)
            if m.status not in (ChatMember.MEMBER, ChatMember.ADMINISTRATOR, ChatMember.OWNER):
                not_joined.append(ch)
...
def kb_after_join_channels(channels_after, cfg):
    """
    ✅ NEW: Set-2 channels ke Join buttons + 'Claim Now' button niche.
    Voice ke saath attach hota hai — koi extra text nahi.
    """
    rows = _build_channel_buttons(channels_after, cfg, ch_set="set2")
    # Claim Now = verify_after_join callback
    raw_label   = apply_emojis(cfg.get("btn_deposit_label", "🎁 Claim Now"), cfg)
    claim_label = strip_tg_emoji(raw_label)
    rows.append([InlineKeyboardButton(claim_label, callback_data="verify_after_join")])
    return InlineKeyboardMarkup(rows)
...
async def send_joined_content(bot, chat_id, user_id, cfg, user_name=""):
    ...
    # Step 2 & 3: Voice + Set-2 channels check
    channels_after = get_channels("channels_after")

    if channels_after:
        # ✅ NEW FLOW: Voice ke saath Set-2 channel join buttons dikhao (koi extra text nahi)
        ch2_kb = kb_after_join_channels(channels_after, cfg)
        if voice_id:
            try:
                # Voice + inline keyboard ek saath — caption = voice_caption setting
                sent = await bot.send_voice(
                    chat_id=chat_id,
                    voice=voice_id,
                    caption=voice_caption,
                    parse_mode=ParseMode.HTML,
                    reply_markup=ch2_kb,  # ← Join buttons directly voice ke niche
                )
                db_set_msg(user_id, sent.message_id)
            except Exception as e:
                logger.warning(f"Voice send failed: {e}")
                # Fallback: sirf text + buttons
                sent = await safe_html_send(bot, chat_id, voice_caption, reply_markup=ch2_kb)
                db_set_msg(user_id, sent.message_id)
        else:
            # Voice set nahi — sirf join buttons
            sent = await bot.send_message(
                chat_id=chat_id,
                text="👇 <b>Niche ke saare channels join karo:</b>",
                reply_markup=ch2_kb,
                parse_mode=ParseMode.HTML,
            )
            db_set_msg(user_id, sent.message_id)
        ...
        # ✅ OLD FLOW: channels_after set nahi — voice + joined_text + deposit button
        if voice_id:
            try:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ✅ SET-1: Voice + Join Buttons (Image 1 style — koi extra text nahi)
        voice_id  = cfg.get("voice_file_id", "")
        voice_cap = apply_emojis(
            cfg.get("voice_caption", "⚠️ Play on Sound 🔊 👆\n👇"), cfg
        )

        if voice_id:
            # Voice set hai — sirf voice + caption + join buttons (koi extra text nahi)
            try:
                sent = await context.bot.send_voice(
                    chat_id=update.effective_chat.id,
                    voice=voice_id,
                    caption=fix_html_entities(voice_cap),
                    parse_mode=ParseMode.HTML,
                    reply_markup=kb,
                )
            except Exception as e:
                logger.warning(f"Set-1 voice send failed: {e}")
                # Fallback: plain caption + buttons
                sent = await update.message.reply_text(voice_cap, reply_markup=kb)
        else:
            # Voice set nahi — purana welcome text + photo fallback
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify_join":
        ...
        if not_joined:
            ...
            voice_id  = cfg2.get("voice_file_id", "")
            voice_cap = apply_emojis(
                cfg2.get("voice_caption", "⚠️ Play on Sound 🔊 👆\n👇"), cfg2
            )

            if voice_id:
                # ── Voice mode: delete old → send fresh voice + buttons ──
                ...
                try:
                    sent = await context.bot.send_voice(
                        ...
                        caption=fix_html_entities(voice_cap),
                        parse_mode=ParseMode.HTML,
                        reply_markup=fixed_kb,
                    )
                    db_set_msg(user.id, sent.message_id)
                except Exception as e:
                    logger.warning(f"Set-1 voice resend failed: {e}")
                    try:
                        sent = await context.bot.send_message(
                            q.message.chat_id, voice_cap, reply_markup=fixed_kb)
                        db_set_msg(user.id, sent.message_id)
                    except Exception:
                        pass
                return
    ...
    # ════════════════════════════════════════════════
    #  DEPOSIT BUTTON CLICK → sirf msg bhejo, koi link nahi
    # ════════════════════════════════════════════════
    if data == "deposit_clicked":
        ...
        await safe_html_send(context.bot, q.message.chat_id, deposit_text)
        return

    # ════════════════════════════════════════════════
    #  VERIFY AFTER JOIN — Set-2 channels verify karo → Deposit dikhao
    # ════════════════════════════════════════════════
    if data == "verify_after_join":
        if db_is_blocked(user.id):
            await q.answer("🚫 Aap block hain.", show_alert=True)
            return

        try:
            not_joined_after = await check_after_not_joined(context.bot, user.id)
        except Exception:
            await q.answer("⚠️ Error aaya! Thodi der baad try karo.", show_alert=True)
            return

        if not_joined_after:
            # Kuch channels abhi bhi join nahi hue
            await q.answer("❌ Pehle saare channels join karo! Phir Claim karo.", show_alert=True)
            # Updated keyboard — channels_after ke buttons dikhao
            cfg2 = cfg_get()
            all_ch2 = get_channels("channels_after")
            try:
                await q.edit_message_reply_markup(
                    reply_markup=kb_after_join_channels(all_ch2, cfg2)
                )
            except Exception:
                pass
            return

        # ✅ Sab Set-2 channels join ho gaye — Congratulations + Deposit button dikhao
        await q.answer()
        cfg2 = cfg_get()

        joined_text2  = cfg2.get("joined_text", "🎊 <b>Congratulations! Reward Unlock Ho Gaya!</b>")
        safe_name2    = user.first_name or user.username or "Dost"
        joined_text2  = joined_text2.replace("{name}", safe_name2)
        joined_text2  = apply_emojis(joined_text2, cfg2)
        joined_photo2 = cfg2.get("joined_photo", "")

        sent = await safe_send_with_photo(
            context.bot, q.message.chat_id,
            joined_text2, photo_id=joined_photo2,
            reply_markup=kb_after_join(cfg2)
        )
        db_set_msg(user.id, sent.message_id)
        return
