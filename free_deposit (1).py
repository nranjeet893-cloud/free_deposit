...
async def send_joined_content(bot, chat_id, user_id, cfg, user_name=""):
    ...
    # Step 3: Joined text + Set-2 Channels + Deposit button ek saath
    # ✅ NEW: channels_after (Set-2) hain to unhe bhi dikhao voice ke baad
    ch2_list     = get_channels("channels_after")
    joined_photo = cfg.get("joined_photo", "")
    if ch2_list:
        # Set-2 channels + deposit button ek keyboard mein
        kb = kb_after_join_with_ch2(cfg, ch2_list)
    else:
        # Set-2 nahi hai to sirf deposit button
        kb = kb_after_join(cfg)
    sent = await safe_send_with_photo(
        bot, chat_id, joined_text, photo_id=joined_photo, reply_markup=kb
    )
