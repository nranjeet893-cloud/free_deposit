...
def init_db():
    with _conn() as c:
        ...
        # ── MIGRATION: button labels premium karo ──
        old_v = c.execute(
            "SELECT value FROM config WHERE key='btn_verify_label'").fetchone()
        if old_v and old_v[0] == "✅ Verify & Claim Reward 🎁":
            c.execute("UPDATE config SET value=? WHERE key='btn_verify_label'",
                      ("╔═「 ✅ VERIFY & CLAIM REWARD 🎁 」═╗",))
        old_d = c.execute(
            "SELECT value FROM config WHERE key='btn_deposit_label'").fetchone()
        if old_d and old_d[0] == "🚀 Deposit Karo & Reward Pao 💰":
