...
def kb_after_join(cfg):
    """Deposit screen — Set-2 channels + deposit button."""
    after_chs = get_channels("channels_after")
    rows = _build_channel_buttons(after_chs, cfg, ch_set="set2")
    raw_label     = apply_emojis(cfg.get("btn_deposit_label", "💰 Deposit Karo & Reward Pao 🚀"), cfg)
    deposit_label = strip_tg_emoji(raw_label)   # ✅ premium emoji → plain
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "deposit_clicked":
        ...
        # ── Popup text (admin se customizable) ──────────────
        ...
        # ── URL button inside popup (admin se customizable) ──
        ...
        if deposit_photo:
            try:
                ...
                await context.bot.send_message(
                    chat_id=q.message.chat_id,
                    text=fix_html_entities(full_text),
                    reply_markup=dep_kb,
                    parse_mode=ParseMode.HTML,
