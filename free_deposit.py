...
def init_db():
    with _conn() as c:
        ...
        # ── MIGRATION: join_btn_style → naam_only force karo ──
        # DB mein "join_naam" tha jisse "JOIN Channel" dikhta tha — fix
        c.execute("UPDATE config SET value='no_name' WHERE key='join_btn_style'")
        c.execute("UPDATE config SET value='🔴' WHERE key='join_btn_prefix'")
...
def _join_btn_label(ch, cfg, idx=0):
    """
    6 advance styles — channel naam kabhi nahi dikhega (by default):
    - no_name  → 🔴 Channel 1
    - fire     → 🔥 Join Now 🔥
    - star     → ⭐ Subscribe Here ⭐
    - arrow    → ➡️ Click Here ⬅️
    - lock     → 🔓 Unlock Channel 1
    - custom   → [admin prefix] 1
    - naam_only→ sirf channel naam (agar admin manually chahein)
    """
    style  = cfg.get("join_btn_style", "no_name")
    prefix = cfg.get("join_btn_prefix", "🔴").strip()
    name   = ch.get("name", "Channel").strip()
    num    = idx + 1

    if style == "no_name":
        return f"{prefix} Channel {num}"
    elif style == "fire":
        return f"🔥 Join Now 🔥"
    elif style == "star":
        return f"⭐ Subscribe Here ⭐"
    elif style == "arrow":
        return f"➡️ Click Here ⬅️"
    elif style == "lock":
        return f"🔓 Unlock Channel {num}"
    elif style == "custom" and prefix:
        return f"{prefix} {num}"
    else:
        return name  # naam_only fallback
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_joined:
        ...
        text = raw_text.replace("{name}", "").replace("Hey , ", "").replace("Hey ,", "").strip()
        sent = await update.message.reply_text(
            text,
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── JOIN BUTTON STYLE ──────────────────────────────
        ...
        await q.edit_message_text(
            ...
            f"Current: <b>{style}</b>\n\n"
            f"🔴 <b>no_name</b>   → <code>🔴 Channel 1</code>\n"
            f"🔥 <b>fire</b>      → <code>🔥 Join Now 🔥</code>\n"
            f"⭐ <b>star</b>      → <code>⭐ Subscribe Here ⭐</code>\n"
            f"➡️ <b>arrow</b>     → <code>➡️ Click Here ⬅️</code>\n"
            f"🔓 <b>lock</b>      → <code>🔓 Unlock Channel 1</code>\n"
            f"🔧 <b>custom</b>    → <code>[prefix] 1</code>\n"
            f"📝 <b>naam_only</b> → <code>Channel ka actual naam</code>",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔴 No Name",     callback_data="admin_jstyle_no_name"),
                 InlineKeyboardButton("🔥 Fire",        callback_data="admin_jstyle_fire")],
                [InlineKeyboardButton("⭐ Star",        callback_data="admin_jstyle_star"),
                 InlineKeyboardButton("➡️ Arrow",       callback_data="admin_jstyle_arrow")],
                [InlineKeyboardButton("🔓 Lock",        callback_data="admin_jstyle_lock"),
                 InlineKeyboardButton("🔧 Custom",      callback_data="admin_jstyle_custom")],
                [InlineKeyboardButton("📝 Naam Only",   callback_data="admin_jstyle_naam_only")],
                [InlineKeyboardButton("🔙 Back",        callback_data="admin_panel")],
            ]), parse_mode=ParseMode.HTML)

    elif data in ("admin_jstyle_no_name", "admin_jstyle_fire", "admin_jstyle_star",
                  "admin_jstyle_arrow", "admin_jstyle_lock", "admin_jstyle_naam_only"):
        style_map = {
            "admin_jstyle_no_name":   "no_name",
            "admin_jstyle_fire":      "fire",
            "admin_jstyle_star":      "star",
            "admin_jstyle_arrow":     "arrow",
            "admin_jstyle_lock":      "lock",
            "admin_jstyle_naam_only": "naam_only",
        }
        chosen = style_map[data]
        cfg_set("join_btn_style", chosen)
        await q.answer(f"✅ Style set: {chosen}", show_alert=True)
        ...
        await q.edit_message_text(
            "✏️ <b>Custom Prefix bhejo:</b>\n\n"
            "Example: <code>🔥</code> → button: <code>🔥 1</code>\n"
            "Koi bhi emoji ya text bhejo:",
