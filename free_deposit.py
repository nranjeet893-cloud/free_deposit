...
def preserve_custom_emojis(text: str, entities) -> str:
    ...
    for ent in sorted_ents:
        if ent.type == "custom_emoji" and getattr(ent, "custom_emoji_id", None):
            ...
            result += f'<tg-emoji emoji-id="{ent.custom_emoji_id}">{fallback}</tg-emoji>'
...
def _build_channel_buttons(channels, cfg, ch_set="set1"):
    """
    ✅ FIX:
    1. URL buttons → directly channel khulta hai, koi naya message nahi (no stacking)
    2. Button label = sirf {emoji} {naam} — 'Join Karo' suffix hataya
    3. Premium emoji strip karo (Telegram buttons mein nahi chalta)
    """
    per_row   = int(cfg.get("btns_per_row", "2"))
    raw_emoji = apply_emojis("{join_emoji}", cfg)
    emoji     = strip_tg_emoji(raw_emoji)   # premium → plain emoji
    rows, pair = [], []
    for i, ch in enumerate(channels):
        label    = _join_btn_label(ch, cfg, idx=i)
        link     = ch.get("link", "")
        btn_text = f"{emoji} {label}"       # ✅ sirf: 👉 JOIN (no "Join Karo")
        if link:
            # URL button → click karo, seedha channel khulta hai (no new message)
            pair.append(InlineKeyboardButton(btn_text, url=link))
        else:
            # Link nahi — callback fallback
...
async def send_joined_content(bot, chat_id, user_id, cfg, user_name=""):
    ...
    sent = await safe_html_send(
        bot, chat_id, joined_text, reply_markup=kb_after_join(cfg)
    )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not_joined:
            ...
            sent = await safe_html_reply(update.message, text, reply_markup=kb)
            db_set_msg(user.id, sent.message_id)
        else:
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify_join":
        ...
        if not_joined:
            ...
            # ✅ FIX: Message EDIT karo — delete+send nahi (stacking band)
            cfg2      = cfg_get()
            ...
            try:
                fixed_text = fix_html_entities(base_text)
                await q.edit_message_text(fixed_text, reply_markup=fixed_kb, parse_mode=ParseMode.HTML)
            except Exception as e:
                err = str(e)
                if "Entity" in err or "entity" in err:
                    import re
                    plain = re.sub(r'<[^>]+>', '', base_text).replace('&amp;', '&')
                    try: await q.edit_message_text(plain, reply_markup=fixed_kb)
                    except: pass
                elif "can't be edited" in err or "MESSAGE_ID_INVALID" in err or "message is not modified" in err.lower():
                    pass  # already correct — kuch karne ki zaroorat nahi
                else:
                    # Fallback: delete + send new
                    try: await q.message.delete()
                    except: pass
                    sent = await safe_html_send(context.bot, q.message.chat_id, base_text, reply_markup=fixed_kb)
                    db_set_msg(user.id, sent.message_id)
            return
    ...
    # ── WELCOME / JOINED TEXT ──────────────────────────
        ...
        await q.edit_message_text(
            "📝 <b>Welcome Text bhejo</b>\n\n"
            "💡 <b>Tips:</b>\n"
            "• Custom emoji directly paste karo — ID nahi chahiye! ✨\n"
            "• Placeholders: <code>{join_emoji}</code> <code>{verify_emoji}</code>\n"
            "• HTML tags allowed: <code>&lt;b&gt;</code> <code>&lt;i&gt;</code>\n"
            "• <code>{name}</code> → user ka naam (optional)",
            reply_markup=kb_back(), parse_mode=ParseMode.HTML)

    elif data == "admin_joined_text":
        context.user_data["awaiting"] = "joined_text"
        await q.edit_message_text(
            "📝 <b>Joined Text bhejo</b>\n\n"
            "💡 <b>Tips:</b>\n"
            "• Custom emoji directly paste karo — ID nahi chahiye! ✨\n"
...
async def msg_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting in key_map:
        # ✅ FIX: Custom emoji (Telegram premium animated) preserve karo
        # Welcome/Joined text mein directly emoji paste karo — ID ki zaroorat nahi!
