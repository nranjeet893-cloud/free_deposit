...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify_join":
        ...
        if not_joined:
            ...
            ch_names = "".join([f"  ❌ <b>{ch.get('name','Channel')}</b>\n" for ch in not_joined])
            error_block = (
                "\n\n━━━━━━━━━━━━━━━━━\n"
                "⚠️ <b>Yeh channels join nahi kiye:</b>\n"
                f"{ch_names}"
                "━━━━━━━━━━━━━━━━━\n"
                "👇 Join karo phir <b>Verify</b> dabao"
            )
            full_text = fix_html_entities(base_text) + error_block
    ...
    if data.startswith("ch_join_"):
        ...
        if idx < len(chs):
            ...
            if link.startswith(("https://", "http://")):
                # ✅ FIX: Sirf clean URL button — koi emoji spam message nahi
                await context.bot.send_message(
                    chat_id=q.message.chat_id,
                    text=f"👇 <b>{name}</b> join karo:",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton(f"➤ {name} Join", url=link)
                    ]]),
                    parse_mode=ParseMode.HTML,
                )
            else:
