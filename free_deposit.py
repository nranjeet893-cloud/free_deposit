...
async def _post_init(application):
    """Called once after Application is fully initialized — good place for startup log."""
...
def main():
    ...
    # ── Step 4: Bot with crash-restart loop ───────────────
    # FIX: Python 3.11 + PTB 20.7 required (see runtime.txt + requirements.txt).
    # Python 3.14 breaks PTB v20 Updater.__slots__ at __init__ time.
    # The simplest correct PTB v20 pattern: build fresh app each restart,
    # but ONLY after a crash — normal runs never hit the loop again.
    while True:
        try:
            app = (
                Application.builder()
            ...
            # run_polling handles its own asyncio event loop cleanly in PTB v20
            app.run_polling(
                drop_pending_updates=True,
                allowed_updates=Update.ALL_TYPES,
                close_loop=False,   # <-- prevents "loop already closed" on restart
            )
        except KeyboardInterrupt:
            logger.info("Bot manually band kiya gaya.")
            break
        except Exception as e:
            logger.error(f"❌ Bot crash: {e} — 5 sec mein restart...")
            time.sleep(5)
