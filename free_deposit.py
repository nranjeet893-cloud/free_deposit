...
async def send_joined_content(bot, chat_id, user_id, cfg, user_name=""):
    ...
    # Step 1: Unlock animation — edit in place for smooth feel
    ...
    # Step 2: Voice note (agar set hai) — sirf voice+caption, no duplicate text
    if voice_id:
        try:
    ...
    # Step 3: Joined text + Deposit keyboard (clean, no sound instructions)
    joined_photo = cfg.get("joined_photo", "")
    sent = await safe_send_with_photo(
        bot, chat_id, joined_text, photo_id=joined_photo, reply_markup=kb_after_join(cfg)
    )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not_joined:
            ...
            # ✅ Button label empty kabhi nahi hoga
            kb = kb_force_join(not_joined, cfg)
            ...
            await send_joined_content(
                context.bot, update.effective_chat.id, user.id, cfg,
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify_join":
        ...
        if not_joined:
            ...
            # ── Step 2: Screen pe HAMESHA message dikhega ──
            cfg2      = cfg_get()
            ...
            # ✅ FIX: Sirf clean welcome text — error block nahi dikhega
            full_text = fix_html_entities(base_text)
            fixed_kb  = kb_force_join(not_joined, cfg2)
    ...
    # ════════════════════════════════════════════════
    #  DEPOSIT BUTTON CLICK — blanket se PEHLE handle karo
    #  (warna q.answer() dob dafa call hone se alert fail hota hai)
    # ════════════════════════════════════════════════
    if data == "deposit_clicked":
        deposit_link  = cfg.get("deposit_link", "").strip()

        if not deposit_link:
            await q.answer("⚠️ Deposit link set nahi hai! Admin se contact karo.", show_alert=True)
            return

        # Dismiss silently (link set hai to sirf message send hoga)
        await q.answer()
        ...
        # ── Deposit message text (admin se customizable, custom emoji support) ──
        popup_text = cfg.get("deposit_text", "").strip()
        if not popup_text:
            popup_text = (
                "🎁 <b>Aapka Special Bonus Ready Hai!</b>\n\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        popup_text = apply_emojis(popup_text, cfg)
        ...
        if register_link:
            ...
            if reg_instr:
                parts.append(apply_emojis(reg_instr, cfg).replace("{register_link}", reg_display))
            else:
                parts.append(f"📌 <b>Register Link:</b>\n{reg_display}")
        if dm_agent_text:
            parts.append(apply_emojis(dm_agent_text, cfg))

        full_text = "\n\n".join(parts)

        # ── Sirf EK button — deposit URL ──
        raw_btn_label = cfg.get("deposit_popup_btn_label", "🚀 Abhi Deposit Karo & Bonus Pao 💰").strip()
        if not raw_btn_label:
            raw_btn_label = "💰 Deposit Karo"
        btn_label = strip_tg_emoji(apply_emojis(raw_btn_label, cfg))
        dep_kb    = InlineKeyboardMarkup([[InlineKeyboardButton(btn_label, url=deposit_link)]])
        ...
        if deposit_photo:
            try:
                await context.bot.send_photo(
                    ...
                    photo=deposit_photo,
                    caption=fix_html_entities(full_text)[:1024],
                    reply_markup=dep_kb,
                    parse_mode=ParseMode.HTML,
                )
            except Exception as e:
                logger.warning(f"Deposit photo send failed ({e}), using text")
                await safe_html_send(context.bot, q.message.chat_id, full_text, reply_markup=dep_kb)
        else:
            await safe_html_send(context.bot, q.message.chat_id, full_text, reply_markup=dep_kb)
        return
    ...
    # ── DEPOSIT ────────────────────────────────────────
        ...
        popup_btn  = cfg.get("deposit_popup_btn_label", "🚀 Abhi Deposit Karo & Bonus Pao 💰")
        dep_txt_ok = "✅ Custom" if cfg.get("deposit_text","") else "📄 Default"
        await q.edit_message_text(
            f"💰 <b>Deposit Settings</b>\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🔗 Deposit Link  : <code>{link[:50]}</code>\n"
            f"📝 Popup Message : {dep_txt_ok}\n"
            f"🏷️ Popup Button  : <code>{popup_btn[:40]}</code>\n"
            f"📌 Register Link : <code>{reg_link[:40]}</code>\n"
            f"💝 DM Agent      : <code>{dm_agent[:40]}</code>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"<i>💡 Deposit button click hone pe popup message aata hai — uska text aur button label yahan se customize karo.</i>",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔗 Deposit Link",      callback_data="admin_set_deposit_link"),
                 InlineKeyboardButton("📝 Popup Message",     callback_data="admin_set_deposit_text")],
                [InlineKeyboardButton("🏷️ Popup Btn Label",   callback_data="admin_set_popup_btn"),
        ...
        cur_disp = cur[:120] if cur else "(Default use ho raha hai)"
        await q.edit_message_text(
            "📝 <b>Deposit Popup Message Set Karo</b>\n\n"
            "Jab user deposit button dabata hai, tab yeh message aata hai.\n\n"
            f"{'✨ <i>Premium emoji set hai!</i>' if has_prem else ''}\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "💡 <b>Premium Emoji:</b> Directly paste karo — sab kaam karta hai!\n"
            "• HTML: <code>&lt;b&gt;</code> <code>&lt;i&gt;</code>\n"
            "• Placeholder: <code>{deposit_emoji}</code>\n\n"
            "Naya message bhejo (premium emoji bhi chalega ✨):",
        ...
        cur_dm = cfg.get("dm_agent_text", "")
        has_prem = "<tg-emoji" in cur_dm
        await q.edit_message_text(
            "💝 <b>DM Agent Text Set Karo</b>\n\n"
            f"{'✨ <i>Premium emoji set hai!</i>' if has_prem else ''}\n"
            "Yeh text deposit popup ke niche aata hai.\n\n"
            "Example:\n"
            "<code>💝DM Agent 👉 @YourAgent</code>\n\n"
            "💡 Premium emoji bhi directly paste kar sakte ho ✨\n\n"
            "Naya text bhejo:",
            reply_markup=kb_back("admin_deposit_menu"), parse_mode=ParseMode.HTML)

    elif data == "admin_set_popup_btn":
        context.user_data["awaiting"] = "deposit_popup_btn_label"
        cur = cfg.get("deposit_popup_btn_label", "🚀 Abhi Deposit Karo & Bonus Pao 💰")
        await q.edit_message_text(
            "🏷️ <b>Deposit Popup Button Label</b>\n\n"
            "Deposit popup message ke andar jo button hota hai (jis pe click karke link khulta hai) — uska label yahan set karo.\n\n"
            f"Current: <code>{cur}</code>\n\n"
            "💡 <b>Tips:</b>\n"
            "• Regular emoji use karo (premium nahi chalega buttons mein)\n"
            "• Short rakho — max 20-25 characters\n\n"
            "Example: <code>💰 Abhi Deposit Karo</code>\n\n"
            "Naya label type karo:",
            reply_markup=kb_back("admin_deposit_menu"), parse_mode=ParseMode.HTML)
