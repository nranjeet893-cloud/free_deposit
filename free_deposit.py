...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── CUSTOM EMOJI MENU ──────────────────────────────
        ...
        await q.edit_message_text(
            ...
            f"🔗 Join emoji : <code>{cfg2.get('join_emoji','🔗')}</code>\n"
            f"💰 Deposit   : <code>{cfg2.get('deposit_emoji','💰')}</code>\n"
            f"🔊 Voice     : <code>{cfg2.get('voice_emoji','🔊')}</code>\n"
            f"✅ Verify    : <code>{cfg2.get('verify_emoji','✅')}</code>\n"
            f"⚡ Alert     : <code>{cfg2.get('alert_emoji','⚡')}</code>",
            reply_markup=InlineKeyboardMarkup([
